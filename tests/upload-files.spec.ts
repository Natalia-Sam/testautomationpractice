...
test.describe("Open main page", () => {
  ...
  test("Select a file", async ({ uploadFilesComp }) => {
    ...
    await expect(uploadFilesComp.confirmFile1Uploaded).toContainText(
  ...
  test("Select multiple files", async ({ uploadFilesComp }) => {
    ...
    await uploadFilesComp.chooseMultipleFiles([
      filePath1,
      filePath2,
      filePath3,
    ]);
    ...
    await expect(uploadFilesComp.confirmFilesUploaded).toContainText(
